...
import { deleteRole, toggleLogin } from '../Config/MasterSlice';

const Header = () => {

  const {role, isLoggedIn, userDetails } = useSelector(state => state.master);
    ...
    <div style={{ backgroundColor: '#006400', height: '18px', width: '100vw' }}></div>
      ...
      <AppBar position="static" sx={{ backgroundColor: '#006400', mt: -4 }}>
        <Toolbar>
          ...
          {
            role === "PUBLIC" ? <Button component={Link} to="/Home" color="inherit">
            HOME
          </Button> : null 
          }
          {
            role === "USER" ? <Button component={Link} to="/Home" color="inherit">
            HOME
          </Button> : null 
          }
          {
            role === "ADMIN" ? <Button component={Link} to="/AdminHome" color="inherit">
            HOME
          </Button> : null 
          }
          {
            role === "USER" ? <Button component={Link} to="/apply" color="inherit">
            Apply Now
          </Button> : null 
          }
          {
            role === "ADMIN" ? <Button component={Link} to="/ManageLoan" color="inherit">
            ManageLoan
          </Button> : null 
          }
          ...
          {/* {
            ...
            >{userDetails.name[0].toUpperCase()}</Avatar>:<Avatar
            sx={{ bgcolor: deepOrange[500] }}
            alt="Remy Sharp"
            src="/broken-image.jpg"
          >{userDetails.name[0].toUpperCase()}</Avatar>) : (<Button component={Link} to="/login" color="inherit">
            Login
          </Button>)
          } */}
          {
  isLoggedIn ? (
    ...
    <>
      ...
      {/* Add any additional login-related elements if needed */}
    </>
  )
}
      ...
      >
        ...
        <List>
          {/* <ListItem component={Link} to="/DrawDash">
            DASHBOARD
          </ListItem> */}
                      {role === "USER" ? (
              <ListItem component={Link} to="/UserDash">
                DASHBOARD
              </ListItem>
            ) : (
              role === "PUBLIC" && (
          ...
          {
            role === "USER" ? <ListItem component={Link} to="/LoanStatus">
            LOAN STATUS
          </ListItem>: null 
          }
          {/* <ListItem component={Link} to="/LoanStatus">
            LOAN STATUS
          </ListItem> */}
          {/* <ListItem component={Link} to="/AdminHome">
            ADMIN
          </ListItem> */}
          {role === "ADMIN" ? (
              <ListItem component={Link} to="/UserDash">
...
{
  isLoggedIn ? (
          <ListItem component={Link} onClick={eventLogout} to="/"> 
            LOGOUT
          </ListItem>):null
}
        </List>
